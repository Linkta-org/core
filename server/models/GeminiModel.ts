...
const model = genAI.getGenerativeModel({
  ...
  /*
   TODO: 1) adding safetySettings
         2) update genAI api route
         3) continue to work on generateResponse in genAi controller
         4) make coresponding changes in userInputController.submitUserInput
  */
});
...
export const startGeneration = async (
  ...
  const chat = model.startChat({ history, generationConfig: generate_config });
